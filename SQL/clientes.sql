...
create table naatik_clientes(
    ...
    servicioInternet int not null,
    foreign key(servicioInternet) references naatik_tipoInternet(internetID),
    ...
    foreign key(contrato) references naatik_tipoContrato(contratoID),
    facturaElectronica tinyint not null,
    metodoPago int not null,
    foreign key(metodoPago) references naatik_tipoPago(pagoID),
